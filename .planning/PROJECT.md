...
**No active milestone** — v6.1 shipped 2026-06-20. Run `/gsd:new-milestone` to start the next cycle.

The auto-fix-from-bug-reports loop is live and validated end-to-end: a human bug report in the v5.0 `BUG_REPORTS` KV channel can be triaged, promoted to a `report-fix-candidate` Issue, turned into a candidate diff against the deterministic matching core, proven regression-safe against the golden corpus, and merged by a human. The fix LLM runs locally via the Claude Code subscription (`npm run fix-report`, ADR-001); CI is notify-only. Open follow-ups: deploy the merged st-ligature matching fix to the extension/webapp/worker (build+deploy decision); finish the minor live-UAT tails (auto-promote issue-close, UAT-03 live cap); the separate v6.0 PROXY_TOKEN dual-token transition.

**Candidate next-milestone themes (v6.2):** PDF-context in fix prompts (PDFCTX-01), LLM triage of `ambiguous` reports (LTRI-01), nightly-cron ingestion (AUTO-01), auto-quarantine of promoted-but-un-goldenized reports (AUTO-02), Worker-route bug fixes (currently classified `infrastructure`, out of fix scope).

## Shipped Milestone: v6.1 Auto-Fix from Bug Reports (Shipped 2026-06-20)

**Goal:** Turn real, human-reported citation failures (the v5.0 `BUG_REPORTS` KV channel) into regression-safe fixes — auto-triage real bugs from noise, analyze them, propose candidate fixes to the matching/normalization core, and prove each fix does not regress other patents before a maintainer merges it. Designed fresh from first principles, grounded entirely in human-reported failures (no autonomous exploration). **Shipped: 5 phases (10-14), 13 plans; pipeline proven live (both regression-gate arms); merged to `main`.**
...
Gave extension users a low-friction in-product affordance to report citation failures, routing rich auto-captured diagnostic bundles to a private Cloudflare-backed observability pipeline (`BUG_REPORTS` KV durable + Discord webhook notify) for maintainer triage — the inbound signal channel that v6.1's resumed auto-fix work will ingest. **Live UAT-01..06 PROVEN against production `pct.tonyrowles.com` before close** (Discord embeds + KV records verified, no IP stored, server-side dedup + cross-browser parity on Chrome/149 + Firefox/151).
...
- ✓ **ING-01..04, TRI-01..07, PROMO-01..04**: Triage layer — `scripts/ingest-reports.mjs` (`workflow_dispatch`-only) reads the `BUG_REPORTS` KV channel via reused `review-reports.mjs` I/O (`wrangler --remote`), heuristically classifies each report through `scripts/report-classifier.mjs` (8 named rules, frozen `CLASSIFICATIONS` enum, golden/quarantine corpus cross-check, every rule Vitest-pinned against real `buildReportPayload()` output), auto- or manually-promotes real bugs into `report-fix-candidate` GitHub Issues via the extracted shared `scripts/gh-client.mjs` (D-05 find-or-create-then-write ordering, `<!-- kv-key: report:{fp}:{ts} -->` dedup, 30-day post-fix suppression), writes `_review.status` back (promoted→triaged, skipped→wontfix), and emits a per-run triage-report artifact — idempotent on re-run. Validated in Phase 11: Triage Layer (3 plans, 2 waves, 46 phase tests; code review found+fixed 3 critical [dedup-marker poisoning, dry-run no-op, un-awaited async main] + 6 warnings)
- ✓ **FIX-01..05, GATE-01..04, COST-01..04**: Fix generation + regression gate — a `report-fix-candidate`-labeled Issue triggers `.github/workflows/v61-report-fix.yml`, which fetches the KV record via `wrangler --remote`, runs the fresh `scripts/report-fix.mjs` dispatcher (D-01) inside a 3-iteration golden+quarantine regression loop, and opens a *draft* PR on `auto-fix/<fp-short>` that the reused `v40-verifier-gate.yml` gates (GATE-03) with no auto-merge anywhere (GATE-04, enforced by a permanent cross-workflow Vitest static-grep invariant). The LLM operates only on the `src/shared/` matching core via the real `REPORT_FIX_SCAFFOLD` prompt (10 FORBIDDEN_PATHS, `<report_data>` untrusted-envelope trust boundary, FORBIDDEN_DELIMITERS-escaped user fields — prompt injection structurally blocked); overfitting (reported `patentNumber` as a `src/` literal) is soft-flagged to `human-review-required` (FIX-04/D-03); all spend routes through `invokeAnthropicSdkWithLedger` with `source:'report-fix-api'` (COST-01), capped at 5 analysis calls/run, with the ledger committed to `ledger-snapshots/report-fix-<fp-short>` before the PR (COST-04 ordering preserved; ledger-branch deviation human-confirmed since ruleset 17086676 blocks bot push-to-main). Validated in Phase 12: Fix Generation + Regression Gate (4 plans, 3 waves, 13/13 reqs + 5/5 success criteria statically verified, 175 phase tests; 5 live-CI behaviors tracked in 12-HUMAN-UAT.md pending real Actions run)
- ✓ **GATE-05**: Triple-gate extension — `assertTripleGate` Leg 3 (`scripts/auto-fix-promote.mjs`) widened to a flat OR accepting `report-fix-candidate` alongside the legacy `triage` label, so a merged v6.1 report-fix PR can drive the existing merge → quarantine → golden-corpus promote cycle. The gate-body change and its PROMOTE-04 byte-unchanged `EXPECTED_BODY` pin (`tests/unit/auto-fix-promote-gate.test.js`) landed in ONE commit (no window where the gate was unenforced); the legacy `triage` path is preserved bit-for-bit and the `auto-fix:partial-verified` rejection + `_skipCiGuard:true` grep-count==1 trust invariants stay green. Source-issue resolvability fixed by emitting `<!-- source_issue: ${{ github.event.issue.number }} -->` from the `v61-report-fix.yml` PR body (D-04 — reuses the unchanged `parseSourceIssue` PREFERRED parse path; `parseSourceIssue`/`runPromote`/`promote-from-quarantine.mjs` untouched). "Fires after merge" is operator `workflow_dispatch` (D-01 — no `pull_request:closed` restored; live dispatch UAT deferred to Phase 14). Validated in Phase 13: Triple-Gate Extension (1 plan, 1 wave, 7/7 must-haves + 3/3 success criteria verified, 77 trust-boundary tests green; code review 0 critical/2 warning/1 info — all test-hardening, implementation reviewer-confirmed secure)

- ✓ **DGST-01** (+ UAT-03 local half): Monday weekly digest gains a GitHub-derived `BUG_REPORTS` section — `renderBugReportsSection` (pure, `<details>` collapsible, locked-order rows: report volume, promoted, open/merged/overfit auto-fix PRs, stuck, promotion rate; degrade-to-`n/a`-while-still-shipping per D-05) + `fetchBugReportIssues` (gh-only, injected-deps seam, errors-returned) wired into `runDigest` after `renderDigest` (outside the ≤50-line budget). GitHub-only source (D-01 — no CF creds in the Monday cron); measures the *promoted* funnel, not raw intake (D-02). Code review found+fixed a blocker (CR-01: both `fetchAutoFixPrs` and `fetchBugReportIssues` used the invalid `gh search prs --json mergedAt` → switched to `gh pr list --state all`; `gh search prs` purged) plus windowing/repo-threading/test-hardening warnings. UAT-03 local half PROVEN in-session: `npm test` exit 0, golden corpus 100% (76 cases / 1789 tests), ledger monthly-cap (HARD_CAP_USD=100) enforcement asserted. Validated in Phase 14: End-to-End UAT + Digest (2 plans, 1 wave, 9/11 must-haves verified in-session, 53 digest tests). **Live pipeline UAT deferred to operator** (UAT-01 full chain, UAT-02 manual-promote, UAT-03 cap-across-Actions) — tracked as 8 pending blocks in `14-HUMAN-UAT.md` (real LLM spend + permanent human-merge gate cannot run in-session; D-08/D-09).
...
---
*Last updated: 2026-06-20 — v6.1 Auto-Fix from Bug Reports SHIPPED + archived (Phases 10-14, 13 plans). Live UAT proved the pipeline end-to-end: report → triage → promote → local subscription LLM fix (ADR-001) → golden-corpus regression gate (both arms — regressing fix reverted, additive st-ligature fix passed) → human merge to main. CI v61-report-fix.yml is notify-only; report-fix runs via `npm run fix-report` on the Claude Code subscription. Earlier history: started milestone v6.1 Auto-Fix from Bug Reports. Human-report-driven auto-fix pipeline rebuilt from first principles: v5.0 `BUG_REPORTS` KV intake → auto-triage (+ manual-promote escape hatch) → analysis → regression-safe candidate fix → human merge gate. Autonomous LLM exploration deferred to future; the v4.3 autonomous machinery (fixture-mutator, `e2e:explore` cron, `v40-auto-fix` synthetic trigger, paused Phases 61–67) is retired/archived in this milestone (supersedes RESUME-V4.3.md). Only the golden-corpus / quarantine regression guard is carried forward. v6.0 Standalone Citation Webapp shipped 2026-06-17 (33/33 reqs, live at cite.tonyrowles.com). Phase 10 (Retirement + Scaffolding) complete 2026-06-17 — autonomous machinery removed, suite green (90 files, golden corpus 100%), `REPORT_FIX_SCAFFOLD` stub stood up for Phase 12. Phase 11 (Triage Layer) complete 2026-06-17 — `ingest-reports.mjs` + `report-classifier.mjs` + shared `gh-client.mjs`; ING/TRI/PROMO (15 reqs) shipped, 16/16 must-haves verified, suite green (95 files, 1671 tests), code review's 3 critical + 6 warning findings fixed. Phase 12 (Fix Generation + Regression Gate) complete 2026-06-18 — `v61-report-fix.yml` + `scripts/report-fix.mjs` dispatcher + `REPORT_FIX_SCAFFOLD` prompt + shared `fix-primitives.js`; FIX/GATE/COST (13 reqs) shipped, 13/13 reqs + 5/5 success criteria statically verified, suite green (98 files, 1757 tests), 5 live-CI behaviors deferred to UAT (12-HUMAN-UAT.md). Phase 13 (Triple-Gate Extension) complete 2026-06-18 — `assertTripleGate` Leg 3 widened to OR-accept `report-fix-candidate` (+ atomic PROMOTE-04 pin, same commit) and the `v61-report-fix.yml` PR body emits the `<!-- source_issue -->` marker so the unchanged `parseSourceIssue` resolves v6.1 source issues; GATE-05 shipped, 7/7 must-haves + 3/3 success criteria verified, 77 trust-boundary tests green, all trust invariants preserved. Phase 14 (End-to-End UAT + Digest) complete 2026-06-18 — DGST-01 `BUG_REPORTS` digest section (gh-only, degrade-to-`n/a`, mirrors `renderAutoFixPipelineSection`) + UAT-03 local half (golden 100%, ledger-cap assertion) shipped and verified in-session; code review found+fixed a blocker (invalid `gh search prs --json mergedAt` → `gh pr list --state all` in both fetch helpers). Verification `human_needed` (9/11) — the live pipeline chain (UAT-01/02 + UAT-03 live half) is operator-deferred by D-08/D-09 and tracked in `14-HUMAN-UAT.md`. **Phase 14 is the v6.1 milestone-closer** — all phases (10-14) complete; the live operator UAT + `/gsd:complete-milestone` remain before v6.1 fully closes.*
