...
- [ ] **PRIV-01**: `src/manifest.firefox.json` `data_collection_permissions` updated — `required: ["websiteActivity"]`, `optional: ["technicalAndInteraction", "websiteContent"]`; selection text is user-controlled via the per-submission [Remove selection text] toggle (CAP-02); November 2025 AMO spec compliance. **Amended Phase 5:** `technicalAndInteraction` MUST be declared in `optional`, not `required` — Mozilla treats technical/interaction data as user-opt-in, and `web-ext lint` (10.4.0) emits `JSON_INVALID` if it appears in `required` (this blocked PRIV-05). Verify PRIV-03 privacy policy + PRIV-04 store-listing describe technical/interaction data as opt-in to stay consistent.
