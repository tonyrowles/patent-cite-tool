 ...
 */
/**
 * In-place patch the built content script so its citation host uses an
 * OPEN shadow root instead of closed. The shadow-open shim only runs in
 * the page's main world via addInitScript; the extension's content
 * script runs in an ISOLATED world that has its own pristine
 * Element.prototype.attachShadow. Playwright's locator engine cannot
 * pierce closed shadow roots (verified Phase 27 verification 2026-05-14),
 * so without this patch the citation pill is invisible to the spec.
 *
 * Patch is idempotent. Production builds are untouched; only dist/ is
 * mutated, which is regenerated by every `npm run build:chrome`.
 */
function patchContentScriptForOpenShadow(extensionPath) {
  const cs = path.join(extensionPath, 'content', 'content.js');
  if (!fs.existsSync(cs)) return;
  const src = fs.readFileSync(cs, 'utf8');
  const patched = src.replace(
    /attachShadow\(\s*\{\s*mode:\s*["']closed["']\s*\}\s*\)/g,
    'attachShadow({ mode: "open" })',
  );
  if (patched !== src) {
    fs.writeFileSync(cs, patched, 'utf8');
  }
}
...
export async function loadExtension({ extensionPath, userDataDir } = {}) {
  ...
  // Make the extension's citation shadow root readable from Playwright.
  patchContentScriptForOpenShadow(extensionPath);
