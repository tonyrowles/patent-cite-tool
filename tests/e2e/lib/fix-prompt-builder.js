 ...
 */
// Standalone export — NOT in PROMPT_SCAFFOLDS (NEVER add it there).
// Purity: no I/O imports; no env reads; same-inputs → byte-identical output.
// Phase 12 FIX-02 / FIX-03 / T-12-04 / T-12-05 / T-12-06 / T-12-07
...
export function buildFixPrompt({ errorClass, issueBody, rewriteHint } = {}) {
  ...
  // Phase 67 PITER-01 — optional rewriteHint append.
  //
  // When the auto-fix dispatcher's in-process iteration loop (PITER-02) re-runs
  // buildFixPrompt on round 1+ after an apply-check-failed or malformed-diff:*
  // outcome, it passes the prior attempt's failure mode (stderr snippet OR
  // parseFencedDiff.reason) as `rewriteHint`. We append the hint AFTER the
  // scaffold returns, under a <prior_attempt_feedback> block, so:
  //
  //   (a) buildScaffoldSystemPrompt body stays byte-unchanged (Phase 45 invariant);
  //   (b) the 7 PROMPT_SCAFFOLDS byte-stability sha256 pins survive by construction
  //       (they hash the thunk return value, NOT this function's return value);
  //   (c) the cache_control hit-rate is preserved on round 0 — when rewriteHint
  //       is undefined/empty, this function returns the byte-identical systemPrompt
  //       as today, so the systemBlocks[0].text cache key is unchanged.
  //
  ...
  if (typeof rewriteHint === 'string' && rewriteHint.length > 0) {
    ...
    systemPrompt = systemPrompt + [
      ...
      '',
      'A prior auto-fix attempt for this same failure was rejected. The verbatim',
      'feedback below is machine-generated by the dispatcher (NOT user input —',
